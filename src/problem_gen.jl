...
function generate_mpc_problem(n::Int, seed::Int = 1)
	...
	P = 2 * sprandn(12, 12, 0.15)
	Q = P' * P + 1e-2 * spdiagm(ones(12))
	Num = 12
	A = generate_double_stochastic_matrix(Num)
	Ad = spdiagm(0.7 * ones(Num)) + 0.3 * A

	Bd = sprandn(Num, 4, 0.15)
	(nx, nu) = size(Bd)
...
function generate_double_stochastic_matrix(n::Int; tol = 1e-6, max_iter = 1000)
	# Initialize a random matrix
	A = sprand(n, n, 0.1)

	function row_normalize!(matrix)
		for i in 1:size(matrix, 1)
			matrix[i, :] ./= (sum(matrix[i, :]) + 1e-5)
		end
	end

	function column_normalize!(matrix)
		for j in 1:size(matrix, 2)
			matrix[:, j] ./= (sum(matrix[:, j]) + 1e-5)
		end
	end
